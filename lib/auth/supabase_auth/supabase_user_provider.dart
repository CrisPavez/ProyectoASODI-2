...
class MiAsistenteASODISupabaseUser extends BaseAuthUser {
  MiAsistenteASODISupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> miAsistenteASODISupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MiAsistenteASODISupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
